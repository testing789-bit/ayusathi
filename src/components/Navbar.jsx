...
import { Heart, Menu, X, MessageCircle, Stethoscope, BookOpen, Bell, Sun, Moon, UserCircle, ChevronDown, Plus, Search, Home } from 'lucide-react';
      ...
      <div className="flex items-center gap-x-4">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="pl-10 pr-3 py-1 rounded bg-blue-800 text-white placeholder-blue-200 focus:outline-none focus:ring-2 focus:ring-blue-400 transition w-48"
            aria-label="Search"
          />
        </div>
        <button
          onClick={() => setDarkMode(!darkMode)}
          className="hidden md:inline-flex p-2 rounded-full hover:bg-blue-800 transition-colors"
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className="flex items-center gap-x-2 focus:outline-none focus:ring-2 focus:ring-blue-400 px-2 py-1 rounded hover:bg-blue-800 transition"
            aria-haspopup="true"
          ...
          {profileOpen && (
            <div className="absolute right-0 mt-2 w-44 bg-white text-blue-900 rounded shadow-lg z-50 border border-blue-100">
              <Link to="/profile" className="block px-4 py-2 hover:bg-blue-50">Profile</Link>
              <Link to="/settings" className="block px-4 py-2 hover:bg-blue-50">Settings</Link>
              <Link to="/logout" className="block px-4 py-2 hover:bg-blue-50">Logout</Link>
            </div>
          )}
        </div>
      </div>
